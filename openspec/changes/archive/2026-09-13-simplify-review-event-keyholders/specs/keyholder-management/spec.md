...
The system SHALL allow an authenticated administrator to assign separate keyholders for event entry and exit using keyholder UUID references. Assignment controls SHALL offer active keyholders by name and SHALL NOT display key numbers. The Review Event assignment section SHALL present the editable entry and exit controls without a redundant read-only assignment summary and SHALL use the shared rounded action styling for `Update Keyholders`.
...
#### Scenario: Administrator reviews event keyholder assignments
