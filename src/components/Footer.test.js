...
describe("application version", () => {
  ...
  test("prefers a supplied release version", () => {
    expect(resolveVersion({ suppliedVersion: "v0.58.0", describe: () => "git-version" })).toBe("v0.58.0");
  });

  test("uses the full Git description when no release version is supplied", () => {
    expect(resolveVersion({ describe: () => "v0.57.0-3-g2b856b6" })).toBe("v0.57.0-3-g2b856b6");
  });

  test("falls back to development when Git metadata is unavailable", () => {
    expect(resolveVersion({ describe: () => { throw new Error("not a Git checkout"); } })).toBe("development");
  });
